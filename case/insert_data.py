...
from .caseParams import insert_data
import env
import json
import unittest
...
class Demo(unittest.TestCase):
    ...
    def test_case(self):
        ...
        """
        for j in range(100):
            for i in insert_data():  # 编辑caseParams.demo方法中的接口参数
                api = 'v1/web/edugarden/info/insert'
                # rep = tools.get_request(api, i)  # 这是get方式的接口请求
                rep = tools.post_request(api, i)  # 这是post方式的接口请求
                print(rep)
                log = function.Logger(config.log_path + 'insert_data.log')  # 需要写入log的文件名
                log.wirte(api, i, rep)
